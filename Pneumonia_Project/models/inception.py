...
class InceptionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.branch2 = nn.Sequential(
            # bottleneck 1×1
            nn.Conv2d(in_channels, b2_reduce, kernel_size=1, bias=False),
            nn.BatchNorm2d(b2_reduce),
            nn.ReLU(inplace=True),
            # 3×3 conv (padding=1 to preserve spatial size)
            nn.Conv2d(b2_reduce, b2_out,padding=1, kernel_size=3),
            nn.BatchNorm2d(b2_out),
            nn.ReLU(inplace=True)
        )
        ...
        self.branch3 = nn.Sequential(
            # bottleneck 1×1
            nn.Conv2d(in_channels, b3_reduce, kernel_size=1, bias=False),
            nn.BatchNorm2d(b3_reduce),
            nn.ReLU(inplace=True),
            # 5×5 conv (padding=2 to preserve spatial size)
            nn.Conv2d(b3_reduce, b3_out, padding=2, kernel_size=5),
        ...
        self.branch4 = nn.Sequential(
            ...
            # 1×1 conv
            nn.Conv2d(in_channels, b4_out, kernel_size=1)
        )
